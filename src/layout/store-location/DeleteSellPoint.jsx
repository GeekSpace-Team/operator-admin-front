...
const DeleteSellPoint = (props) => {
  const [open, setOpen] = React.useState(false);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);
  const classes = makeStyles(ownStyles)();

  const deleteSellPoint = async () => {
    await AxiosInstance.delete(
      `/admin/delete-sell-point?unique_id=${props.unique_id}`
    )
      .then((response) => {
      ...
      >
        <Box sx={style}>
          <Typography className={classes.text}>
            Siz <em>{props.name} </em> atly hasabyňyzy hakykatdanam
...
export default DeleteSellPoint;
