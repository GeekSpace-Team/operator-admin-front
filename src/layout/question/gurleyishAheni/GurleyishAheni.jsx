...
const GurleyishAheni = () => {
  ...
  const getData = async () => {
    await AxiosInstance.get("/admin/get-info?type=speak_accent")
      .then((result) => {
        if (!result.data.err) {
  ...
  const deleteByPosition = (id) => {
    if (window.confirm("Siz çyndanam pozmakçymy?")) {
      AxiosInstance.delete(`/admin/delete-info?id=${id}&type=speak_accent`)
        .then((result) => {
